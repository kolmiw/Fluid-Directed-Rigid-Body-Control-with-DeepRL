...
class MPMSolver:
    ...
    @ti.kernel
    def seed_sphere(self, new_particles: ti.i32, new_material: ti.i32, color: ti.i32):
        for i in range(self.n_particles[None],
                       self.n_particles[None] + new_particles):
            self.material[i] = new_material
            x = ti.Vector.zero(ti.f32, self.dim)
            r = ti.random() ** (1.0/self.dim) * self.source_bound[1][0]
            theta = 2 * ti.math.pi * ti.random()
            if i == self.n_particles[None]:
                r = 0
            elif i == self.n_particles[None] + 1:
                r = 1
                theta = 0
            x[0] = ti.math.cos(theta)
            x[1] = ti.math.sin(theta)

            x = self.source_bound[0] + x * r
            self.seed_particle(i, x, new_material, color,
                               self.source_velocity[None], None)
    
    @ti.kernel
    def seed_sphere_3D(self, new_particles: ti.i32, new_material: ti.i32, color: ti.i32):
        for i in range(self.n_particles[None],
                       self.n_particles[None] + new_particles):
            self.material[i] = new_material
            x = ti.Vector.zero(ti.f32, self.dim)
            ...
            phi = ti.math.acos(1 - 2 * ti.random())
            x[0] = ti.math.sin(phi) * ti.math.cos(theta)
            x[1] = ti.math.sin(phi) * ti.math.sin(theta)
            x[2] = ti.math.cos(phi)
    ...
    def add_sphere(self,
        ...
        if sample_density is None:
            sample_density = 2**self.dim
        vol = radius**2 * ti.math.pi
        if self.dim == 3:
            vol = vol * radius * 4.0 / 3.0
        num_new_particles = int(sample_density * vol / self.dx**self.dim + 1)
        assert self.n_particles[None] + num_new_particles <= self.max_num_particles

        for i in range(self.dim):
            self.source_bound[0][i] = center[i]
            self.source_bound[1][i] = radius

        self.set_source_velocity(velocity=velocity)
        if self.dim == 2:
            self.seed_sphere(num_new_particles, material, color)
        else:
            self.seed_sphere_3D(num_new_particles, material, color)
        self.n_particles[None] += num_new_particles
